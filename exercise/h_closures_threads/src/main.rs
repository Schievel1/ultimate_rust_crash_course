...
fn main() {
    ...
    let handle_b = thread::spawn(move || {
        pause_ms(100);
        for msg in rx {
            println!("Thead B says: {}", msg);
        }
    });

    for i in 0..=10 {
        print!("Main thread sends {} to ", i);
        if i % 2 == 0 { tx.send(i).unwrap(); println!("Thread A")}
        else { tx2.send(i).unwrap(); println!("Thread B")}
    }
